resource "rke_cluster" "leads-cluster" {
...
resource "local_file" "kube_cluster_yaml" {
  ...
  content  = rke_cluster.leads-cluster.kube_config_yaml
}

resource "local_file" "rke_cluster_yaml" {
  filename = "${path.root}/cluster.yml"
  content  = rke_cluster.leads-cluster.rke_cluster_yaml
}

resource "local_file" "rke_state" {
  filename = "${path.root}/cluster.rkestate"
  content  = rke_cluster.leads-cluster.rke_state
}